...
- 모바일 요청보다 서버 조사가 먼저 종료되도록 research timeout을 55초로 제한
...
- 외부 리셀 플랫폼의 비공식 내부 API에 의존하지 않고 공개 검색 근거만 사용
...
8. 리셀 제품은 같은 흐름에서 사이즈를 선택하고, exact-size 근거가 확인되는 경우에만 가격 표시
