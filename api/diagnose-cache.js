...
export default async function handler(req, res) {
  ...
  const before = (payload.offers || []).map(offer => ({ seller: offer.seller, sources: (offer.sources || []).map(source => ({ host: host(source.url), title: source.title || '' })) }));
  const aligned = alignOfferSources(payload);
  const after = (aligned.offers || []).map(offer => ({ seller: offer.seller, sources: (offer.sources || []).map(source => ({ host: host(source.url), title: source.title || '' })) }));
