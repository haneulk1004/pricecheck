...
test('research request uses low thinking with structured Google Search', () => {
  ...
  assert.match(request.input, /inline URL citation/);
...
test('incomplete interactions are never parsed as complete JSON', () => {
  const text = '{"offers":[{"seller":"전자랜드"';
  const converted = interactionToGenerateContent({
    status: 'incomplete',
    steps: [{ type: 'model_output', content: [{ type: 'text', text, annotations: [{ type: 'url_citation', url: 'https://example.com', title: 'example.com', start_index: 0, end_index: 10 }] }] }]
  });
  assert.throws(
