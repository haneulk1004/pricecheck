...
export async function redisCommand(command, env, fetchImpl = fetch) {
  try {
    ...
    if (!response.ok || data.error || !Object.hasOwn(data, 'result')) throw new Error('redis');
    return data.result;
  } catch { throw new ResearchError(503, 'CACHE_UNAVAILABLE', '조사 사용량을 확인할 수 없습니다. 잠시 후 다시 시도해주세요.'); }
}
...
export function createResearchHandler({ env = process.env, fetchImpl = fetch, log = console.info } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      if (!['GEMINI_API_KEY', 'UPSTASH_REDIS_REST_URL', 'UPSTASH_REDIS_REST_TOKEN', 'IP_HASH_SECRET'].every(key => env[key]?.trim()) || env.IP_HASH_SECRET.length < 32 || !/^https:\/\//.test(env.UPSTASH_REDIS_REST_URL)) throw new ResearchError(503, 'CONFIG_UNAVAILABLE', '가격 조사 설정을 확인 중입니다. 잠시 후 다시 시도해주세요.');
      const key = cacheKey(input);
      phase = 'admission';
      const ipHash = hashClientIP(req, env);
      const admission = await redisCommand(['EVAL', ADMIT_SCRIPT, '3', key, `${PREFIX}:ip:${ipHash}`, `${PREFIX}:global`], env, fetchImpl);
      ...
      if (!response.ok) {
        log(JSON.stringify({ event: 'price_research', outcome: 'provider_error', status: response.status }));
      ...
      phase = 'cache_write';
      await redisCommand(['SET', key, JSON.stringify(payload), 'EX', String(CACHE_SECONDS)], env, fetchImpl);
