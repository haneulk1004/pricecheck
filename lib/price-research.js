...
export function createResearchHandler({ env = process.env, fetchImpl = fetch, log = console.info } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const secretTooShort = Boolean(env.IP_HASH_SECRET && env.IP_HASH_SECRET.length < 32);
      const invalidRedisUrl = Boolean(env.UPSTASH_REDIS_REST_URL && !/^https:\/\//.test(env.UPSTASH_REDIS_REST_URL));
      if (missing.length || secretTooShort || invalidRedisUrl) {
        log(JSON.stringify({ event: 'price_research_config', missing, secretTooShort, invalidRedisUrl }));
