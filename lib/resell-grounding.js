...
function resellPrompt(input) {
  return [
    ...
    'Use DIRECT product/listing/market pages as price evidence.',
    'For KREAM, use the canonical /products/... product page only. Do NOT use KREAM STYLE, social posts, user posts, discovery pages, tagged outfit posts, or community content as price evidence.',
    'For other resale platforms, do not use search-result pages, community posts, editorial pages, or unrelated product pages.',
    'A source is valid only when the SAME source page supports the exact model/product, the requested size, and the exact returned KRW price.',
    'Prioritize publicly indexed KREAM and SOLDOUT product pages, then other public Korean resale product pages.',
    ...
    'Never estimate, invent, infer, or transfer a price from a similar product. If exact citable evidence is unavailable, return an empty offers array.',
...
export function buildResellInteractionRequest(generateContentBody, input, model) {
  ...
  return {
    ...
    system_instruction: 'Use Google Search for current resale research. Accept only direct product/listing pages where the exact model, requested size, and exact returned price are supported on that same page. Never use STYLE/social/community pages as price evidence. Return no offers when exact evidence is missing.',
  ...
  for (const step of steps) {
    ...
    for (const block of step.content) {
      ...
      for (const rawAnnotation of Array.isArray(block.annotations) ? block.annotations : []) {
        ...
        // Interactions API citation indices are UTF-8 byte offsets, not JS character indices.
