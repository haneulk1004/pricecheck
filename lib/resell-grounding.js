...
const RESEARCH_TIMEOUT_MS = 75000;
const CITATION_FALLBACK_TIMEOUT_MS = 60000;
...
export function buildResearchInteractionRequest(generateContentBody, input, model) {
  ...
  return {
    ...
    system_instruction: systemInstruction(input),
...
function buildCitationFallbackRequest(generateContentBody, input, model) {
  const maxOutputTokens = generateContentBody?.generationConfig?.maxOutputTokens ?? 4096;
  return {
    model,
    input: [
      researchPrompt(input),
      'Citation recovery pass: the prior structured response omitted URL citation annotations.',
      'Use Google Search again and return ONLY one raw JSON object in the requested offers shape, with no Markdown fence and no prose outside JSON.',
      'Attach inline URL citations directly to every price claim. If you cannot attach a URL citation to an exact price, omit that offer.'
    ].join(' '),
    system_instruction: `${systemInstruction(input)} URL citation annotations on exact price claims are mandatory. Return raw JSON only.`,
    tools: [{ type: 'google_search', search_types: ['web_search'] }],
    generation_config: { max_output_tokens: maxOutputTokens },
    store: false
  };
...
function interactionHasOffers(interaction) {
  const steps = Array.isArray(interaction?.steps) ? interaction.steps : [];
  for (const step of steps) {
    ...
    for (const block of step.content) {
      ...
      try {
        ...
        if (Array.isArray(parsed?.offers) && parsed.offers.length > 0) return true;
      } catch {
        // Structured primary output should be JSON. Non-JSON output does not trigger a paid retry.
      }
    }
  }
  return false;
...
export function interactionToGenerateContent(interaction) {
  ...
  const hasModelText = parts.some(part => typeof part.text === 'string' && part.text.trim());
  const usableOutput = interaction?.status === 'completed' || (interaction?.status === 'incomplete' && hasModelText && groundingSupports.length > 0);
  return {
    candidates: [{
      finishReason: usableOutput ? 'STOP' : 'OTHER',
      content: { parts },
      ...(hasGrounding ? {
        groundingMetadata: {
...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    console.info(JSON.stringify({ event: 'research_grounding_request', mode: input.mode, model, hasSize: Boolean(input?.size), hasModelCode: Boolean(input?.modelCode), modelCode: input?.modelCode || null }));
    ...
    let interaction = await interactionResponse.json();
    let diagnostics = interactionDiagnostics(interaction);
    console.info(JSON.stringify({ event: 'research_grounding_interaction', mode: input.mode, ...diagnostics }));

    if (diagnostics.annotationCount === 0 && diagnostics.status === 'completed' && interactionHasOffers(interaction)) {
      console.info(JSON.stringify({ event: 'research_grounding_citation_fallback', mode: input.mode, outcome: 'started' }));
      try {
        const fallbackResponse = await fetchImpl(INTERACTIONS_URL, {
          ...options,
          signal: AbortSignal.timeout(CITATION_FALLBACK_TIMEOUT_MS),
          body: JSON.stringify(buildCitationFallbackRequest(generateContentBody, input, model))
        });
        if (fallbackResponse.ok) {
          const fallbackInteraction = await fallbackResponse.json();
          const fallbackDiagnostics = interactionDiagnostics(fallbackInteraction);
          console.info(JSON.stringify({ event: 'research_grounding_citation_fallback', mode: input.mode, outcome: 'completed', ...fallbackDiagnostics }));
          if (fallbackDiagnostics.annotationCount > 0) {
            interaction = fallbackInteraction;
            interactionResponse = fallbackResponse;
          }
        } else {
          console.info(JSON.stringify({ event: 'research_grounding_citation_fallback', mode: input.mode, outcome: 'provider_error', status: fallbackResponse.status }));
        }
      } catch (error) {
        console.info(JSON.stringify({ event: 'research_grounding_citation_fallback', mode: input.mode, outcome: 'transport_error', name: error?.name ?? 'Error' }));
      }
    }
