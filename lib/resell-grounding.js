...
export function buildResellInteractionRequest(generateContentBody, input, model) {
  ...
  return {
    ...
    system_instruction: 'Use the available Google Search tool for current resale price research. Return only evidence-backed prices with citations; otherwise return no offers.',
...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    console.info(JSON.stringify({ event: 'resell_grounding_request', model, hasSize: Boolean(input?.size), hasModelCode: Boolean(input?.modelCode) }));
