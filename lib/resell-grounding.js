...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    console.info(JSON.stringify({ event: 'research_grounding_request', mode: input.mode, model, hasSize: Boolean(input?.size), hasModelCode: Boolean(input?.modelCode), modelCode: input?.modelCode || null, thinkingLevel: 'low', structuredOutput: true }));
