...
function searchResultDiagnostics(interaction) {
  const steps = Array.isArray(interaction?.steps) ? interaction.steps : [];
  const results = [];
  for (const step of steps) {
    if (step?.type !== 'google_search_result') continue;
    const items = Array.isArray(step.result) ? step.result : step?.result ? [step.result] : [];
    for (const item of items.slice(0, 5)) {
      if (!item || typeof item !== 'object') continue;
      const keys = Object.keys(item).sort().slice(0, 20);
      results.push({
        keys,
        hasUrl: typeof item.url === 'string',
        hasUri: typeof item.uri === 'string',
        hasTitle: typeof item.title === 'string',
        hasSnippet: typeof item.snippet === 'string',
        hasSearchSuggestions: typeof (item.search_suggestions ?? item.searchSuggestions) === 'string'
      });
    }
  }
...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    try {
      ...
      console.info(JSON.stringify({ event: 'resell_grounding_transport_error', name: error?.name ?? 'Error' }));
      throw error;
    }

    if (!interactionResponse.ok) {
      console.info(JSON.stringify({ event: 'resell_grounding_provider_error', status: interactionResponse.status }));
      return interactionResponse;
    }

    const interaction = await interactionResponse.json();
    console.info(JSON.stringify({ event: 'resell_grounding_interaction', ...interactionDiagnostics(interaction) }));
    console.info(JSON.stringify({ event: 'resell_grounding_search_result_shape', results: searchResultDiagnostics(interaction) }));
