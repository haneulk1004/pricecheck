...
function researchPrompt(input) {
  if (input.mode === 'resell') {
    const kreamQuery = `site:kream.co.kr/products "${input.modelCode || input.productName}" "${input.size}" "거래가"`;
    return [
      ...
      `For KREAM, explicitly run a targeted search equivalent to: ${kreamQuery}. Inspect the canonical KREAM product result and its completed-trade rows before concluding that no exact-size evidence exists.`,
...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    console.info(JSON.stringify({ event: 'research_grounding_request', mode: input.mode, model, hasSize: Boolean(input?.size), hasModelCode: Boolean(input?.modelCode), modelCode: input?.modelCode || null, citationProfile: 'proven-v4', maxOutputTokens: 8192 }));
