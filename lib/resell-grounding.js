...
function researchPrompt(input) {
  const outputRule = [
    'Return ONLY one raw JSON object with no Markdown fence and no prose outside JSON.',
    'The JSON shape is {"offers":[{"seller":"...","productName":"...","priceKRW":123000,"condition":"...","note":"..."}]}.',
    'Return at most 5 KRW offers.',
    'For every offer, the inline URL citation covering priceKRW MUST come from that seller or marketplace own domain.',
    'Never label an offer as Seller A when the cited price source belongs to Seller B, a comparison site, community page, unrelated site, or another marketplace.',
    'If seller-domain evidence and the exact numeric price cannot both be cited, omit that offer.',
    'Use Korean descriptions.'
  ].join(' ');
  ...
  return [
    ...
    'Use only current publicly listed prices for NEW retail products from the exact seller product page or a directly matching seller-owned shopping page.',
    ...
    'If exact product + exact current KRW price + seller-owned source cannot all be supported, omit that offer. If no offer qualifies, return an empty offers array.',
    outputRule,
    `Product data: ${JSON.stringify(input)}`
  ].join(' ');
}
...
function systemInstruction(input) {
  return input.mode === 'resell'
    ? 'Use Google Search for current resale research. Enforce exact product, exact model code, exact selected-size evidence, and seller-domain citation alignment. Reject headline/overall prices, other-size prices, and cross-seller citations. Return raw JSON only; otherwise return an empty offers array.'
    : 'Use Google Search for current Korean retail research. Enforce exact product matching, current new-retail evidence, and seller-domain citation alignment. Reject cross-seller citations. Return raw JSON only; otherwise return an empty offers array.';
}

export function buildResearchInteractionRequest(generateContentBody, input, model) {
  const requestedMax = generateContentBody?.generationConfig?.maxOutputTokens ?? 4096;
  const maxOutputTokens = Math.min(Math.max(Number(requestedMax) || 4096, 1024), 4096);
  return {
    model,
...
export function createPromptAwareFetch(fetchImpl = fetch) {
  return async function promptAwareFetch(url, options = {}) {
    ...
    console.info(JSON.stringify({ event: 'research_grounding_request', mode: input.mode, model, hasSize: Boolean(input?.size), hasModelCode: Boolean(input?.modelCode), modelCode: input?.modelCode || null, thinkingLevel: 'low', structuredOutput: false }));
